...
class FriendlinkController extends AuthController {
    ...
    public function index(){
        ...
        $this->cur_v='Friendlink-index';

        $page="Friendlink/index";
        $page_buttons=M('PageButtons')->where(array('page'=>$page))->select();
        $this->page_buttons=$page_buttons;
        $this->page=$page;

        $type= M('Type')->where(array('pid'=>1393))->select();
        $this->type=$type;
    ...
    public function ajax_add(){
        ...
        if($data){
            ...
            if($id){
                $row= D('Friendlink')->relation(true)->find($id);
    ...
    public function ajax_del(){
        ...
        if($id){
            $res=M('Friendlink')->where(array('id'=>$id))->delete();
